...
public class InventoryProvider extends ContentProvider {
    ...
    public Uri insertProduct(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(InventoryContract.InventoryEntry.COL_NAME);
        if (name == null) {
            throw new IllegalArgumentException("Product requires a name");
        }

        // Check that the gender is valid
        Integer quantity = values.getAsInteger(InventoryContract.InventoryEntry.COL_QUANTITY);

        // If the weight is provided, check that it's greater than or equal to 0 kg
        Float price = values.getAsFloat(InventoryContract.InventoryEntry.COL_PRICE);
    ...
    /**
     * Update nventory in the database with the given content values. Apply the changes to the rows
